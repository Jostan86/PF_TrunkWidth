...
class PFEngine:
    ...
    def reset_pf(self, setup_data: dict) -> None:
        """Reset the particle filter with the given setup data."""

        self.start_pose_center = np.array(setup_data['start_pose_center'])
        self.start_pose_width = setup_data['start_pose_width']
        self.start_pose_height = setup_data['start_pose_height']
        self.rotation = setup_data['rotation']
        ...
        self.R = np.diag([setup_data['r_dist'], setup_data['r_angle']]) ** 2
        self.dist_sd = setup_data['dist_sd']
        ...
        self.bin_angle = setup_data['bin_angle']
    ...
    def save_odom(self, x_odom, theta_odom, time_stamp, num_readings=1):
    ...
    def scan_update(self, tree_msg):
        ...
        if tree_msg['positions'] is not None:
            ...
            # Calculate the 'best' particle as the average of all the particles
